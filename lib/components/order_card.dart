      ...
      child: Row(
        ...
        children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            Text("Order #${order.orderId}", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
            const SizedBox(height: 4),
            Text("${order.itemsCount} items", style: TextStyle(color: subTextColor)),
            const SizedBox(height: 12),
            Row(children: [
              Text(order.date ?? "\$${order.price}", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: order.date != null ? subTextColor : textColor)),
              const SizedBox(width: 10),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: order.statusColor, borderRadius: BorderRadius.circular(8)),
                child: Text(order.status, style: TextStyle(color: order.statusTextColor, fontWeight: FontWeight.bold, fontSize: 12)),
              ),
            ]),
                    ...
                    showDialog(
                      ...
                      builder: (ctx) => AlertDialog(
                        ...
                        actions: [
                          ...
                          ElevatedButton(
                            onPressed: () {
                               Provider.of<RewardsProvider>(context, listen: false).deductPoints(20);
                               Provider.of<OrderProvider>(context, listen: false).cancelOrder(order.orderId);
                               Provider.of<NotificationProvider>(context, listen: false).addNotification(
                                 NotificationModel(
                                   title: "Order Cancelled ❌",
                                   subtitle: "Your order #${order.orderId} (${order.itemsCount} items) has been cancelled.",
          ...
          Container(
            width: 70, height: 70,
            decoration: BoxDecoration(color: imgBg, borderRadius: BorderRadius.circular(12)),
            child: ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.asset("assets/vitamin_c.png", fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Icon(Icons.medication, color: subTextColor, size: 30))),
          ),
        ],
      ),
    );
